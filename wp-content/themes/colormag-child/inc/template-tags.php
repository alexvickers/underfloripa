...
<span class="cm-author cm-vcard">
	...
	<?php
	// Get coauthors from ACF
	$coauthors = get_field( 'coauthors' );

	if ( $coauthors ) {
		echo ' + '; // separator — can be customized or replaced with a span or comma
		foreach ( $coauthors as $user ) {
			$user_url = get_author_posts_url( $user->ID );
			$user_name = esc_html( $user->display_name );
			$user_title = esc_attr( $user->display_name );

			echo '<a class="url fn n" href="' . esc_url( $user_url ) . '" title="' . $user_title . '">' . $user_name . '</a>';

			// Optional: Add comma if more than one coauthor
			if ( $user !== end( $coauthors ) ) {
				echo ', ';
			}
		}
	}
