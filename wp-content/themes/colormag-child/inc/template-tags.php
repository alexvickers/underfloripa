...
if ( ! function_exists( 'colormag_date_meta_markup' ) ) :
	...
	/**
	 * Prints post meta markup for published and last updated date/time.
	 *
	 * @return void
	 */
	function colormag_date_meta_markup() {
		...
		if ( get_the_modified_time( 'U' ) !== get_the_time( 'U' ) ) {
			$modified = sprintf(
				'<time class="cm-modified-date updated" datetime="%1$s">%2$s</time>',
				esc_attr( get_the_modified_date( 'c' ) ),
				esc_html__( 'Última atualização: ', 'colormag' ) . esc_html( get_the_modified_date() . ' at ' . get_the_modified_time() )
			);
		}

		printf(
			'<span class="cm-post-date"><a href="%1$s" title="%2$s" rel="bookmark">%3$s %4$s %5$s</a></span>',
